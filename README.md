...
Цель проекта заключалась в реализации своей версии стандартной функции языка `C` printf с некоторыми его спецификаторами.
...
В аргументы функции первым параметром передаётся дескриптор файла, затем форматная строка со спецификаторами, а в последующие - передаются аргументы в порядке упоминания спецификаторов, для обработки которых они используются.

Для вызова стоит использовать обёртку из макроса:
...
``` pseudocode
    ...
    RBX := мантисса (RAX)
    RBX := RBX + 2 ^ (длина мантиссы)
    RDX := экспонента (RAX)

    RCX := количество нулей на конце двоичной записи RBX
    shr RBX, RCX
    ...
    Иначе
    {
        ...
        RDX := количество незначащих нулей в двоичной записи RBX
        ...
        Иначе
        {
            ...
            // использовано, равно разнице (3*RCX - RDX)/4,
            // так как при каждом использовании приближения
...
Вторым символом после C идёт буква, обозначающая выбранный цвет, в который будет разукрашен текст до следующей смены цвета или до конца выводимого текста.

В случае, если выходной файл - не стандартный вывод, то он описывается в разметке **HTML**. В него записываются `<html>` - в начале и `</html>` - в конце. Так же после тега `<html>` указывается чёрный шрифт текста, а затем при переключении цвета шрифта указывается завершение обработки прошлого шрифта и начало обработки нового. В конце файла так же указывается завершение обработки шрифта.

| Символ для спецификатора | Цвет, который он обозначает
| :---:                    | :---:
| b                        | <p style="color: black">  Чёрный  ⬤</p>
| g                        | <p style="color: green">  Зелёный 🟢</p>
| r                        | <p style="color: red">    Красный 🔴</p>
| w                        | <p style="color: grey">   Белый   ◯</p>
| y                        | <p style="color: yellow"> Жёлтый  🟡</p>
